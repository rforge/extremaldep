...
##
## THIS FUNCTIONS ARE TAKEN FROM THE PACKAGE CompRandFld  AND ARE DEFINED INTERNALLY
##
    ...
    if(from=='Gev')
      {
        ...
        Dist2Dist <- .C('Dist2Dist', as.double(data), as.double(param[1,]), as.double(param[2,]),
                        ...
                        res=double(numdata * numcoord), PACKAGE='ExtremalDep', DUP=TRUE, NAOK=TRUE)$res
      }
...
GevLogLik <- function(data, numdata, param)
  {
    ...
    result <- .C('GevLogLik', as.double(data), as.integer(numdata), as.double(param),
                 res=double(1), PACKAGE='ExtremalDep', DUP=TRUE, NAOK=TRUE)$res
    return(result)
  }
